...
HMIPB_Uchar8Type    GPIOLimitateIO[HMIPBTaskNumber] = {HMIPB_UNKN};
...
void ScanImplement_HMIPushButton(void)
{
    if(pxReadyLeads.uxNumberOfItems != 0)
    {
        ...
        {
            pxTaskDispatch_HMIPushButton(pxCurrentTCB,GPIOLimitateIO[pxCurrentTCB->_tasksequence]);
...
{
    for(int i = 0; i < HMIPBTaskNumber; i++)
    {
        GPIOLimitateIO[i] = HMIPB_UNKN;
    }
